...
export const getStaticProps = ({}) => getSnippets([RateLimitPy, RateLimitTs]);
...
<GithubSnippet src={RateLimitPy} target="Workflow" />

Next, let's add a task that utilizes a dynamic rate limit:
...
<GithubSnippet src={RateLimitTs} target="Workflow" />
