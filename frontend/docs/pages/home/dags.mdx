...
export const DagGo = {
  path: "examples/v1/workflows/dag.go",
};

export const getStaticProps = ({}) => getSnippets([DagGo]);
...
<GithubSnippet src={DagGo} target="Declaring a Workflow" />
...
<GithubSnippet src={DagGo} target="Defining a Task" />
...
<GithubSnippet src={DagGo} target="Adding a Task with a parent" />
